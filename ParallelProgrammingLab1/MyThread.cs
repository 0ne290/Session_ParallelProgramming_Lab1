...
public class MyThread
{
    ...
    public static void PrintPrefixes(StreamWriter outputFile)
    {
        foreach (var thread in Threads)
        {
            var prefix = string.Empty;
            
            for (var i = 0; i < thread._quantity; i++)
            {
                foreach (var semaphore in thread._semaphores)
                    for (var j = 0; j < thread.CpuBurst; j++)
                        prefix += $"InQueue {semaphore.Name}, Running {semaphore.Name}, ";
            }
            
            prefix += "Completed";
            
            outputFile.WriteLine($"\t{thread.Name} = {prefix}");
        }
    }
    ...
    {
        foreach (var thread in Threads)
            outputFile.WriteLine($"\t{thread.Name}: <{thread._protocol}>");
    }
